...
public class ExamplesOrganizationClonerCEImpl implements ExamplesOrganizationClonerCE {
    ...
    private Mono<UpdateResult> forkThemes(Application srcApplication, Application destApplication) {
        return Mono.zip(
                themeService.cloneThemeToApplication(srcApplication.getEditModeThemeId(), destApplication.getId()),
                themeService.cloneThemeToApplication(srcApplication.getPublishedModeThemeId(), destApplication.getId())
        ).flatMap(themes -> {
